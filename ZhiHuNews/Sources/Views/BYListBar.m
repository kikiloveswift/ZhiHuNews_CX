...
-(void)setVisibleItemList:(NSMutableArray *)visibleItemList{
    ...
    if (!self.btnBackView) {
        CGFloat itemW = visibleItemList.count > 0 ? [self calculateSizeWithFont:itemFont Text:visibleItemList[0]].size.width : 26;
        self.btnBackView = [[UIView alloc] initWithFrame:CGRectMake(10,(self.frame.size.height-20)/2,itemW + kExtraPadding,20)];
        self.btnBackView.backgroundColor = UIColorRGB(202.0, 51.0, 54.0,1);
        self.btnBackView.layer.cornerRadius = 5;
        [self addSubview:self.btnBackView];
        
        self.maxWidth = 20;
        self.backgroundColor = UIColorRGB(238.0, 238.0, 238.0, 1);
...
-(void)makeItemWithTitle:(NSString *)title{
    ...
    [item setTitleColor:UIColorRGB(111.0, 111.0, 111.0, 1) forState:0];
    [item setTitleColor:UIColorRGB(111.0, 111.0, 111.0, 1) forState:1<<0];
...
-(void)itemClick:(UIButton *)sender{
    if (self.btnSelect != sender) {
        [self.btnSelect setTitleColor:UIColorRGB(111.0, 111.0, 111.0, 1) forState:0];
        ...
        if ([self.itemDelegate respondsToSelector:@selector(itemClickWith:)])
        {
            [self.itemDelegate itemClickWith:sender.titleLabel.text];
        }
    }
